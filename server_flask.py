...
def receive_data_for_table( list_of_symbol=None ):
    ...
    try:
        ...
        url_full_path = url_full_path[s] +"&api_token=a5bU3AxoLKNcgBMMSqVJoLRYvPhyw6l2J55ucjjk2GQ9sTxT0t7YI8FdkxBo"
        if app_is_cheat:
...
def settings():
   ...
   # form.symbols_list.data = str_of_symbols[👎] # not working
   if request.method == 'GET':
       try:
           # res = requests.get(app_settings_url)
           # str_of_symbols=json.loads(res.text)[0].get("symbol")+" "
       ...
       return render_template('settings.html', form = form , title='StockApp', start_symbols = str_of_symbols[s])
   elif request.method == 'POST':
